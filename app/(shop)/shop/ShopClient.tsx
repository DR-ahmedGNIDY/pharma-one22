...
// Real Category.slug values (auto-generated from the Arabic names above) —
// used to ask the API for just that category's products instead of the whole catalog.
...
function ShopContent({ initialProducts }: { initialProducts: Product[] }) {
  ...
  // Re-fetch, scoped to the requested category, whenever it changes in the URL
  // (instead of always loading the entire catalog and filtering client-side).
  // The server already rendered products for the current category, so the
  // first run of this effect must not refetch them. Only a category change
  // made after hydration triggers a request.
  const initialCategoryRef = useRef(categoryParam);
  useEffect(() => {
    if (initialCategoryRef.current === categoryParam) return;
  ...
  const loadProducts = async (categoryKey: string | null) => {
    setLoading(true);
    try {
      const categorySlug = categoryKey ? catSlugMap[categoryKey] : undefined;
            ...
            {loading ? (
              <div className={`grid gap-5 ${
                ...
                {Array.from({ length: PAGE_SIZE }).map((_, index) => (
                  <div key={index} className="luxury-card aspect-[3/4] animate-pulse bg-black-light" />
                ))}
              </div>
